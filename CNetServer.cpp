...
void CNetServer::runIoThread()
{
	...
	while (1)
	{
		...
		if (session == nullptr) // session°ú ¹«°üÇÑ ºñµ¿±â ¿äÃ»
		{
		...
		if (cbTransferred == 0 || session->disconnect) // IO ½ÇÆÐ È¤Àº PostQueue ºñµ¿±â ¿äÃ»
		{
			...
			case static_cast<unsigned short>(ePost::CUSTOM_EVENT_PEND): // DB ÀúÀå µî.. session release Àü¿¡ Ã³¸®µÇ¾ß ÇÏ´Â ÀÛ¾÷µé
			{
				/* Todo: Session ³» Queue·Î ½Äº°ÀÚ Àü´Þ¹Þ±â. 
				   °¡»óÇÔ¼ö ±¸ÇöÀ¸·Î Ã³¸®ÇÒ ³»¿ë »ç¿ëÀÚ¿¡°Ô ¸Ã±â±â. */
...
void CNetServer::writeLog()
{
	...
	while (log == nullptr)
	{
		m_logQueue.Dequeue(&log); // Enqueue ¶§¹®¿¡ ½ÇÆÐÇÒ ¼ö ÀÖÀ½ 
