...
bool CNetClient::Connect(const wchar_t* serverIp, unsigned short port,
	int iocpWorker, int iocpActive, bool nagle, unsigned char packet_key, unsigned char packet_code)
{
	bool ret = false;
...
void CNetClient::ReleaseClient(void)
{
	WSACleanup();
	for (int i = 0; i < m_iocpWorkerNum; i++)
...
unsigned long _stdcall CNetClient::IoThread(void* param)
{
	CNetClient* client = (CNetClient*)param;
...
inline void CNetClient::RunIoThread()
{
	int ret_gqcp;
...
bool CNetClient::ConnectSession()
{
	bool ret;
...
inline void CNetClient::DisconnectSession()
{
	if (m_disconnect == 0)
	{
...
inline bool CNetClient::RecvPost()
{
...
inline void CNetClient::SendPost()
{
...
inline int CNetClient::UpdateIOCount()
{
	int temp;
...
inline void CNetClient::UpdatePendCount()
{
	// disconnect 한번에 확인
	int temp;
...
inline void CNetClient::Release()
{
